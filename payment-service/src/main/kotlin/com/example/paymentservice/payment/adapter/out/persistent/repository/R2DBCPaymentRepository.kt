...
class R2DBCPaymentRepository (
    ...
    private fun insertPaymentOrders(
        ...
        val valueClauses = paymentEvent.paymentOrders.joinToString(", ") {
            paymentOrder -> "($paymentEventId, ${paymentOrder.sellerId}, " +
                "'${paymentOrder.orderId}', ${paymentOrder.productId}, " +
                "${paymentOrder.amount}, '${paymentOrder.paymentStatus}')"
        }
        ...
        val SELECT_PENDING_PAYMENT_QUERY =
            """
                SELECT pe.id as payment_event_id, 
                       pe.payment_key, pe.order_id, 
                       po.id as payment_order_id, 
                       po.payment_order_status, po.amount, 
                       po.failed_count, po.threshold
